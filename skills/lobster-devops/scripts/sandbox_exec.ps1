#!/usr/bin/env pwsh
# ========================================
# lobster-devops sandbox wrapper (Windows) — "public image" version
# Per M2 2026-07-08: sandbox-first dev policy
# Image: alpine:latest (Docker Hub official)
...
$ROLE = "lobster-devops"
$IMAGE = if ($env:LOBSTERAI_IMAGE) { $env:LOBSTERAI_IMAGE } else { "alpine:latest" }
...
if ($args.Count -eq 0) {
    Write-Error "[sandbox] $ROLE requires at least 1 arg (e.g., kubectl version --client)"
    exit 2
}
