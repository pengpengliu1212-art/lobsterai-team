#!/usr/bin/env pwsh
# ========================================
# lobster-pm sandbox wrapper (Windows) — "public image" version
# Per M2 2026-07-08: sandbox-first dev policy
# Image: python:3.12-alpine (Docker Hub official)
# Tools needed (pre-installed in image): python3, pip
# Tools we install in workspace (if missing): none (use upstream image)
# ========================================

$ErrorActionPreference = "Stop"

$ROLE = "lobster-pm"
# Public image (per SANDBOX_RULES.md): Docker Hub official, no custom build
...
# Input validation
if ($args.Count -eq 0) {
    Write-Error "[sandbox] $ROLE requires at least 1 arg (e.g., python3 --version)"
    exit 2
}

# Production mode: pin by digest (immutable)
# Dev/CI mode (default): use mutable tag (flexible)
