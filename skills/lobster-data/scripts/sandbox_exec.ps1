#!/usr/bin/env pwsh
# ========================================
# lobster-data sandbox wrapper (Windows) — "public image" version
# Per M2 2026-07-08: sandbox-first dev policy
# Image: python:3.12-alpine (Docker Hub official)
# Per M3 review 2026-07-08: seccomp=default + production digest pinning
# ========================================

$ErrorActionPreference = "Stop"

$ROLE = "lobster-data"
$IMAGE = if ($env:LOBSTERAI_IMAGE) { $env:LOBSTERAI_IMAGE } else { "python:3.12-alpine" }
...
if ($args.Count -eq 0) {
    Write-Error "[sandbox] $ROLE requires at least 1 arg (e.g., python3 -c 'import sys; print(sys.version)')"
    exit 2
}
...
# Pull image if missing (with proper error handling)
...
# Data libs are installed in /workspace/.libs (writable, persists across runs)
# Set PYTHONPATH to include them. Wrapper does NOT pre-install — user can:
#   docker exec -it <container> pip install --target /workspace/.libs <pkg>
# Or: PYTHONPATH=/workspace/.libs python3 -c "import pandas"
$dockerArgs = @(
    "run", "--rm",
    "--read-only",
